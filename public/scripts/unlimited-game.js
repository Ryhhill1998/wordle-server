...
// Get game colours
var colour1 = JSON.parse(localStorage.getItem("colour1")) ?? "blue";
var colour2 = JSON.parse(localStorage.getItem("colour2")) ?? "yellow";

// Get list of saved scores
// localStorage.removeItem("mainScoresList");
var mainScoresList = JSON.parse(localStorage.getItem("mainScoresList")) ?? [];

// Get number of times played
// localStorage.removeItem("mainTimesPlayed");
var mainTimesPlayed = parseInt(JSON.parse(localStorage.getItem("mainTimesPlayed")) ?? "0");

// Get current streak of wins player is on
// localStorage.removeItem("mainCurrentStreak");
var mainCurrentStreak = parseInt(JSON.parse(localStorage.getItem("mainCurrentStreak")) ?? "0");

// Get max streak of wins player has had
// localStorage.removeItem("mainMaxStreak");
var mainMaxStreak = parseInt(JSON.parse(localStorage.getItem("mainMaxStreak")) ?? "0");

// Get the player's scores distribution from won games
// localStorage.removeItem("mainScoreDistribution");
var mainScoreDistribution = JSON.parse(localStorage.getItem("mainScoreDistribution")) ?? [0, 0, 0, 0, 0, 0];
...
$("#game-stats").click(function() {
  ...
  createChart(mainScoreDistribution, chartColours);
  showStats();
  $("#options-menu").fadeOut(300);
...
function gameLost() {
  ...
  mainTimesPlayed++;
  localStorage.setItem("mainTimesPlayed", JSON.stringify(mainTimesPlayed));

  mainCurrentStreak = 0;

  localStorage.setItem("mainCurrentStreak", JSON.stringify(mainCurrentStreak));

  var winPercentage = Math.round(100 * (mainScoresList.length / mainTimesPlayed));

  $(".games-played").text(mainTimesPlayed);
  $(".win-percent").text(winPercentage);
  $(".current-streak").text(mainCurrentStreak);
  $(".max-streak").text(mainMaxStreak);
...
function gameWon() {
  ...
  mainScoresList.push(currentScore);
  localStorage.setItem("mainScoresList", JSON.stringify(mainScoresList));
  mainTimesPlayed++;
  localStorage.setItem("mainTimesPlayed", JSON.stringify(mainTimesPlayed));
  mainCurrentStreak++;

  if (mainCurrentStreak > mainMaxStreak) {
    mainMaxStreak = mainCurrentStreak;
    localStorage.setItem("mainMaxStreak", JSON.stringify(mainMaxStreak));
  }

  localStorage.setItem("mainCurrentStreak", JSON.stringify(mainCurrentStreak));

  mainScoreDistribution[gameRound - 2]++;
  localStorage.setItem("mainScoreDistribution", JSON.stringify(mainScoreDistribution));
